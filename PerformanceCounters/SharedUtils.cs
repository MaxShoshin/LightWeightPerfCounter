...
namespace LightWeight.PerformanceCounters
{
    internal static class SharedUtils
    {
        ...
        internal static void EnterMutexWithoutGlobal(string mutexName, ref Mutex mutex)
        {
            ...
            var tmpMutex = new Mutex(false, mutexName, out createdNew);
        ...
        private static bool SafeWaitForMutexOnce(Mutex mutexIn, ref Mutex mutexOut)
        {
            ...
            finally
            {
                ...
                var result = Interop.Interop.Kernel32.WaitForSingleObject(mutexIn.SafeWaitHandle, 500);
                switch (result)
                {
        ...
        internal static string GetLatestBuildDllDirectory()
        {
            var dllDir = "";
            ...
            try
            {
                ...
                if (complusReg != null)
                {
                    var installRoot = (string)complusReg.GetValue("InstallRoot");
                    if (installRoot != null && installRoot != string.Empty)
                    {
                        ...
                        var versionPrefix = "v" + Environment.Version.Major + "." + Environment.Version.Minor;
                        var policyKey = complusReg.OpenSubKey("policy");
                        ...
                        if (policyKey != null)
                        {
                            try
                            {
                                ...
                                var bestKey = policyKey.OpenSubKey(versionPrefix);
                                ...
                                else
                                {
                                    ...
                                    // installed.
                                    var majorVersions = policyKey.GetSubKeyNames();
                                    int[] largestVersion = { -1, -1, -1 };
                                    for (var i = 0; i < majorVersions.Length; i++)
                                    {

                                        var majorVersion = majorVersions[i];
                                        ...
                                        {
                                            int[] currentVersion = { -1, -1, -1 };

                                            var splitVersion = majorVersion.Substring(1).Split('.');
                                            ...
                                            var k = policyKey.OpenSubKey(majorVersion);
                                            if (k == null)
                                            {
                            ...
                            {
                                var installBuilder = new StringBuilder();
        ...
        private static int GetLargestBuildNumberFromKey(RegistryKey rootKey)
        {
            ...
            var minorVersions = rootKey.GetValueNames();
            for (var i = 0; i < minorVersions.Length; i++)
            {
                int o;
